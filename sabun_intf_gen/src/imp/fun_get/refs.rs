...
pub fn get_ref_fun_string(id : &str, snake_name : &str, is_old : bool, var_type : VarType, self_mod_name: &str, proxy_name : &str, result_type_name: &str) -> String{
    ...
    push(&mut s, 1,&format!("\
    let ref_id = match qv{{
        Qv::None =>{{ return {}::Null; }},",option_type_str(var_type)));
    push(&mut s, 2,&format!("\
        Qv::Undefined =>{{ return {}::Undefined; }},",option_type_str(var_type)));
    push(&mut s, 2,&format!("Qv::Val(id) =>id,\
    }};"));
    push(&mut s, 1,&format!("let root = unsafe{{ self.root.as_ref().unwrap() }};"));
    push(&mut s, 1,&format!("let ref_ptr : *const {} = root.{}().from_id(ref_id);", id, result_type_name));
