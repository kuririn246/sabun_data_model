...
pub struct InnerMutSource{
    stem : String,
    is_old : bool,
    item_source : MutItemSource,
}
impl InnerMutSource{
    pub fn new(stem : String, is_old : bool, item_source : MutItemSource) -> InnerMutSource{
        InnerMutSource{ stem, is_old, item_source }
    }
    pub fn from(desc : &MemberDesc) -> InnerMutSource {
    ...
    pub fn get(&self, mod_name : &str, ptr_exp : &str) -> String{
        ...
        sb.push(1,&format!("let ans = {}::get_inner_mut_list({}, \"{}\").unwrap();", &mod_name, ptr_exp, id));
