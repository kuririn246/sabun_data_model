...
//
// impl InnerDataPtr {
//     pub fn new(data : *const InnerData, list_def : *const ListDefObj, root : *const RootObject) -> InnerDataPtr { InnerDataPtr { data, list_def, root }}
//     pub fn data(&self) -> *const InnerData{ self.data }
//     pub fn list_def(&self) -> *const ListDefObj{ self.list_def }
// }
//
// pub fn get_kvs(ps: InnerDataPtr) -> DataKVs {
//     let (data,list_def) = unsafe{ (ps.data.as_ref().unwrap(), ps.list_def.as_ref().unwrap()) };
//     get_kvs_impl(list_def, data.list(), data.old())
// }
//
// pub fn get_value(ps : InnerDataPtr, id : &str) -> Option<ListItemPtr>{
//     let (data, list_def) = unsafe{ (ps.data.as_ref().unwrap(), ps.list_def.as_ref().unwrap()) };
