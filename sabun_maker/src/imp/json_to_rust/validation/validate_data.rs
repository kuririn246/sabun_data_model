...
pub fn validate_data(def : &ListDefObj, data_map : &HashMap<String, ListItem>, root : &RootObject, can_ref_old : bool, names : &Names) -> Result<()>{
    for (name, val) in data_map{
        //name==old なものがあっても別にかまわない。消すと互換性が崩れるだろう
        validate_list_item(def, val.values(), val.refs(), root, can_ref_old, &names.append(name))?
    }
    return Ok(());
}